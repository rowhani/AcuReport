...
acuSgIsoError   = "ERROR from acuSgIso module"

#===========================================================================
#
# "acuSgIso": IsoSurface actor
#
#===========================================================================

class AcuSgIso:
    '''
	class AcuSgIso creates objects of IsoSurface actors
    '''

    def __init__( self, parent  ):
	'''
	    AcuSgIso is used to create Iso-surface actors.
...
#---------------------------------------------------------------------------
# addIsoSet: Add an iso-surface actor to the scene graph
#---------------------------------------------------------------------------

    def addIsoSet( self, isoObj, isoVec, isoVal, name  ):
        '''
	    Add an iso-surface actor to the scene graph

	    Arguments:
	        isoObj	    - acuiso object
		isoVec	    - iso-surface vector
		isoVal	    - iso-surface value
		name	    - an optional name for iso-surface actor

	    Output:
	        actor       - iso-surface actor
        '''
	...
	if type( isoVec ) == types.StringType and \
        ...
        if type( isoVec ) != numarray.numarraycore.NumArray or \
           isoVec.shape[0] != self.parent.crd.shape[0]:
            raise acuSgIsoError, "Invalid type or name for isoVec."        

	self.isoVec     = isoVec
        (isoCrd,isoTris,isoQuads,isoPrj) = \
                                isoObj.getIsoSurfaces( isoVec, isoVal   )
        
        if not len( isoTris ) and not len( isoQuads ):
            raise acuSgIsoError, "The isoTris and isoQuads are both empty"
	...
	if len( isoTris ):
        ...
        actor           = self.sceneGraph.addIsoSrf( isoCrd,        cnn,
                                                     topology,      name,
                                                     self.disp,     self.clr,
...
#---------------------------------------------------------------------------
# removeIsoSet: Remove an iso-surface actors from the scene graph
#---------------------------------------------------------------------------

    def removeIsoSet( self  ):
        '''
        Remove an iso-surface actors from the scene graph
        ...
        for remActor in self.actList:
            self.sceneGraph.remIsoActor(            remActor            )
        self.actList    = [] 

#---------------------------------------------------------------------------
# setIsoVal: Change\Set the value of the iso-surface
#---------------------------------------------------------------------------

    def setIsoVal( self, newVal  ):
        '''
	    Change\Set the value of the iso-surface
	    
	    Arguments:
	        newVal  - The new value that the iso-surface is set to
        ...
        self.removeIsoSet(                                              )
        self.addIsoSet(             self.isoObj,    self.isoVec,
                                    newVal,         self.name           )

#---------------------------------------------------------------------------
# display: Get\Set the display type of the iso-surface actor
#---------------------------------------------------------------------------

    def display( self, style = None ):
        '''
	    Get\Set the display type of the iso-surface actor
	    
	    Arguments:
	        style   - The desired display type of the iso-surface actor
...
#---------------------------------------------------------------------------
# transparencyVal: Get\Set the transparency value of the iso-surface actor
#---------------------------------------------------------------------------

    def transparencyVal( self, value = None ):
        '''
	    Get\Set the transparency value of the iso-surface actor
	    
	    Arguments:
	        value   - The desired transparency value of the iso-surface actor
...
#---------------------------------------------------------------------------
# transparency: Get\Set the transparency of the iso-surface actor
#---------------------------------------------------------------------------

    def transparency( self, trans = None ):
        '''
	    Get\Set the transparency of the iso-surface actor
...
#---------------------------------------------------------------------------
# color: Get\Set the color of the iso-surface actor
#---------------------------------------------------------------------------

    def color( self, r = None, g = None, b = None ):
        '''
	    Get\Set the color of the iso-surface actor
...
#---------------------------------------------------------------------------
# visibility: Get\Set the visibility of the iso-surface actor
#---------------------------------------------------------------------------

    def visibility( self, vis = None ):
        '''
	    Get\Set the visibility of the iso-surface actor
...
#---------------------------------------------------------------------------
# setVisibility: Set the visibility of the iso-surface actor
#---------------------------------------------------------------------------
...
#---------------------------------------------------------------------------
# lineWidth: Get\Set the line width of the iso-surface actor
#---------------------------------------------------------------------------

    def lineWidth( self, width = None ):
        '''
	    Get\Set the line width of the iso-surface actor
...
#---------------------------------------------------------------------------
# pointSize: Get\Set the point size of the iso-surface actor
#---------------------------------------------------------------------------

    def pointSize( self, size = None ):
        '''
	    Get\Set the point size of the iso-surface actor
